#!/usr/bin/env python3
"""
Settings migration utility for NeutArr
...
def migrate_settings():
    ...
    try:
        ...
        for app in ["sonarr", "radarr", "lidarr", "readarr"]:
            if app in settings and "neutarr" in settings[app]:
                logger.info(f"Found nested neutarr section in {app}, migrating...")

                # Move all settings from app.neutarr to app level
                for key, value in settings[app]["neutarr"].items():
                    if key not in settings[app]:
                        settings[app][key] = value
                        logger.info(f"Moved {app}.neutarr.{key} to {app}.{key}")

                # Remove the neutarr section
                del settings[app]["neutarr"]
                logger.info(f"Removed {app}.neutarr section")
                changes_made = True
