...
class WebUrlFilter(logging.Filter):
    ...
    def filter(self, record):
        ...
        if isinstance(record.msg, str):
            ...
            record.msg = re.sub(r'(http|https)://[^\s<>"]+', "[REDACTED URL]", record.msg)
...
def apply_log_filters():
    ...
    # Apply to neutarr logger
    neutarr_logger = logging.getLogger("neutarr")
    for handler in neutarr_logger.handlers:
        handler.addFilter(web_filter)
