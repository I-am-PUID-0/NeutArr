...
def check_connection(app_type: str = None) -> bool:
    ...
    if not api_url:
        logger.error(
            f"API URL is not configured for {current_app_type} in settings. Please set it up in the Settings page."
        )
        return False

    if not api_key:
        logger.error(
            f"API Key is not configured for {current_app_type} in settings. Please set it up in the Settings page."
        )
        return False
    ...
    try:
        ...
        if response.status_code == 401:
            logger.error(
                f"Connection test failed: 401 Client Error: Unauthorized - Invalid API key for {current_app_type.title()}"
            )
            return False
...
def get_missing_episodes(pageSize: int = 1000) -> Optional[Dict]:
    ...
    if result:
        ...
        if result.get("records"):
            logger.debug(
                f"First few missing episodes: {result['records'][:2] if len(result['records']) > 2 else result['records']}"
            )
    else:
