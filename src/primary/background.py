#!/usr/bin/env python3
"""
NeutArr - Main entry point for the application
...
def app_specific_loop(app_type: str) -> None:
    ...
    try:
        ...
        try:
            ...
            app_logger.debug(
                f"'get_configured_instances' not found in {app_module.__name__}. Assuming single instance mode."
            )
        ...
        get_queue_size = getattr(
            api_module, "get_download_queue_size", lambda api_url, api_key, api_timeout: 0
    ...
    app_logger = logging.getLogger(f"neutarr.{app_type}")
            ...
            if max_queue_size >= 0:
                try:
                    ...
                    if current_queue_size >= max_queue_size:
                        ...
                        app_logger.info(
                            f"Queue size ({current_queue_size}) is below maximum ({max_queue_size}). Proceeding."
                        )
                except Exception as e:
                    app_logger.warning(
            ...
            if hunt_upgrade_enabled and process_upgrades:
                try:
                    ...
                    if app_type == "sonarr":
                        ...
                        # For other apps that still use the old signature
                        processed_upgrades = process_upgrades(
                            app_settings=combined_settings, stop_check=stop_check_func
                        )
        ...
        while elapsed < sleep_seconds:
            ...
            if os.path.exists(reset_file_path):
                try:
                    ...
                    app_logger.info(
                        f"!!! RESET FILE DETECTED !!! Manual cycle reset triggered for {app_type} (timestamp: {timestamp}). Starting new cycle immediately."
                    )
        ...
        if is_configured:
            ...
            if app_type not in app_threads or not app_threads[app_type].is_alive():
                ...
                thread = threading.Thread(
                    target=app_specific_loop, args=(app_type,), name=f"{app_type}-Loop", daemon=True
                )
            ...
            logger.warning(
                f"{app_type} is no longer configured. Thread will likely stop after failing connection checks."
            )
...
def instance_list_generator_loop():
    ...
    while not stop_event.is_set():
        try:
            ...
            logger.debug(
                f"Generated instance list with {sum(len(apps) for apps in instances.values())} total instances"
            )
        except Exception as e:
...
def start_instance_list_generator():
    ...
    instance_list_generator_thread = threading.Thread(
        target=instance_list_generator_loop, name="InstanceListGenerator", daemon=True
    )
...
def start_hourly_cap_scheduler():
    ...
    hourly_cap_scheduler_thread = threading.Thread(
        target=hourly_cap_scheduler_loop, name="HourlyCapScheduler", daemon=True
    )
...
def start_neutarr():
    ...
    if os.environ.get("NEUTARR_RUN_MIGRATION", "false").lower() == "true":
        logger.info("Running settings migration from neutarr.json (if found)...")
        settings_manager.migrate_from_neutarr_json()
    ...
    try:
        ...
        shutdown_threads()
        logger.info("--- NeutArr Background Tasks stopped --- ")
