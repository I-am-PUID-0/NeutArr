...
def test_connection():
    ...
    try:
        ...
        try:
            ...
            if version.startswith("3"):
                ...
                return jsonify(
                    {
                        "success": False,
                        "message": f"Connected to unknown version {version}, but NeutArr requires Eros V3",
                    }
                ), 400
        ...
        return jsonify(
            {"success": False, "message": "Failed to connect. Check that the URL is correct and that Eros is running."}
        ), 503
    ...
    try:
        ...
        for instance in instances:
            ...
            enabled_instances.append(
                {"api_url": api_url, "api_key": api_key, "instance_name": instance_name, "api_timeout": api_timeout}
            )
