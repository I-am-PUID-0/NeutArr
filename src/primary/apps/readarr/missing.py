...
def process_missing_books(
    ...
    readarr_logger.info(
        f"Found {len(unprocessed_authors)} unprocessed authors out of {len(author_ids)} total authors with missing books."
    )

    if not unprocessed_authors:
        readarr_logger.info(
            f"No unprocessed authors found for {instance_name}. All available authors have been processed."
        )
        return False
    ...
    authors_to_process = random.sample(unprocessed_authors, min(hunt_missing_books, len(unprocessed_authors)))  # nosec B311
    ...
    for author_id in authors_to_process:
        ...
        author_name = (
            author_info.get("authorName", f"Author ID {author_id}") if author_info else f"Author ID {author_id}"
        )
        ...
        log_message = (
            f"Triggering Book Search for {len(book_details)} books by author '{author_name}': [{details_string}]"
        )
        ...
        if search_command_result:
            ...
            command_id = (
                search_command_result.get("id") if isinstance(search_command_result, dict) else search_command_result
            )
            readarr_logger.info(
                f"Triggered book search command {command_id} for author {author_name}. Assuming success for now."
            )  # Log only command ID
            ...
            readarr_logger.debug(
                f"Logged history entries for {len(books_by_author[author_id])} books by author: {author_name}"
            )
            ...
            readarr_logger.info(
                f"Processed {processed_count}/{len(authors_to_process)} authors/groups for missing books this cycle."
            )
        else:
    ...
    if processed_authors:
        ...
        readarr_logger.info(
            f'Completed processing {processed_count} authors/groups for missing books this cycle: "{authors_list}"'
        )
    else:
