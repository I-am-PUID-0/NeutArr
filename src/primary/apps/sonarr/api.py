...
def get_series(
    api_url: str, api_key: str, api_timeout: int, series_id: Optional[int] = None
...
def get_calendar(
    api_url: str, api_key: str, api_timeout: int, start_date: Optional[str] = None, end_date: Optional[str] = None
) -> List:
    """
...
def get_missing_episodes(
    api_url: str, api_key: str, api_timeout: int, monitored_only: bool, series_id: Optional[int] = None
) -> List[Dict[str, Any]]:
    ...
    while True:
        ...
        while retry_count <= retries_per_page and not success:
            ...
            params = {"page": page, "pageSize": page_size, "includeSeries": "true"}
            ...
            sonarr_logger.debug(
                f"Requesting missing episodes page {page} (attempt {retry_count + 1}/{retries_per_page + 1})"
            )
            ...
            try:
                ...
                if not response.content:
                    sonarr_logger.warning(
                        f"Empty response for missing episodes page {page} (attempt {retry_count + 1})"
                    )
                ...
                try:
                    ...
                    if total_records_on_page < page_size:
                        sonarr_logger.debug(
                            f"Received {total_records_on_page} records (less than page size {page_size}). Last page."
                        )
                        success = True
                        break  # Exit retry loop, then also exit pagination loop
                    ...
                    sonarr_logger.error(
                        f"Failed to decode JSON response for missing episodes page {page} (attempt {retry_count + 1}): {e}"
                    )
                    if retry_count < retries_per_page:
                        retry_count += 1
                        time.sleep(retry_delay)
                ...
                sonarr_logger.error(
                    f"Unexpected error for missing episodes page {page} (attempt {retry_count + 1}): {e}"
                )
                if retry_count < retries_per_page:
                    retry_count += 1
                    time.sleep(retry_delay)
    ...
    if monitored_only:
        ...
        filtered_missing = [
            ep
            for ep in all_missing_episodes
            if ep.get("series", {}).get("monitored", False) and ep.get("monitored", False)
        ]
        sonarr_logger.debug(
            f"Filtered for monitored_only=True: {len(filtered_missing)} monitored episodes (out of {original_count} total)"
        )
...
def get_cutoff_unmet_episodes(
    ...
    while True:
        ...
        while retry_count <= retries_per_page and not success:
            ...
            sonarr_logger.debug(
                f"Requesting cutoff unmet page {page} (attempt {retry_count + 1}/{retries_per_page + 1})"
            )

            try:
                response = requests.get(url, headers={"X-Api-Key": api_key}, params=params, timeout=api_timeout)
                sonarr_logger.debug(
                    f"Sonarr API response status code for cutoff unmet page {page}: {response.status_code}"
                )
                response.raise_for_status()  # Check for HTTP errors

                if not response.content:
                    sonarr_logger.warning(
                        f"Empty response for cutoff unmet episodes page {page} (attempt {retry_count + 1})"
                    )
                    if retry_count < retries_per_page:
                        retry_count += 1
                        time.sleep(retry_delay)
                ...
                try:
                    ...
                    sonarr_logger.error(
                        f"Failed to decode JSON for cutoff unmet page {page} (attempt {retry_count + 1}): {e}"
                    )
                    if retry_count < retries_per_page:
                        retry_count += 1
                        time.sleep(retry_delay)
                ...
                sonarr_logger.error(
                    f"Request error for cutoff unmet page {page} (attempt {retry_count + 1}): {error_details}"
                )
                if retry_count < retries_per_page:
                    retry_count += 1
                    time.sleep(retry_delay)
                ...
                sonarr_logger.error(
                    f"Unexpected error for cutoff unmet page {page} (attempt {retry_count + 1}): {e}", exc_info=True
                )
                if retry_count < retries_per_page:
                    retry_count += 1
                    time.sleep(retry_delay)
    ...
    if monitored_only:
        ...
        filtered_cutoff_unmet = [
            ep for ep in all_cutoff_unmet if ep.get("series", {}).get("monitored", False) and ep.get("monitored", False)
        ]
        sonarr_logger.debug(
            f"Filtered for monitored_only=True: {len(filtered_cutoff_unmet)} monitored cutoff unmet episodes remain (out of {original_count} total)."
        )
...
def get_cutoff_unmet_episodes_random_page(
    api_url: str, api_key: str, api_timeout: int, monitored_only: bool, count: int
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        random_page = random.randint(1, total_pages)  # nosec B311
        ...
        params = {"page": random_page, "pageSize": page_size, "includeSeries": "true"}
        ...
        if monitored_only:
            filtered_records = [
                ep for ep in records if ep.get("series", {}).get("monitored", False) and ep.get("monitored", False)
            ]
        ...
        if len(records) > count:
            selected_records = random.sample(records, count)  # nosec B311
            ...
            sonarr_logger.debug(
                f"Returning all {len(records)} episodes from page {random_page} (fewer than requested {count})"
            )
            return records
...
def get_missing_episodes_random_page(
    api_url: str, api_key: str, api_timeout: int, monitored_only: bool, count: int, series_id: Optional[int] = None
) -> List[Dict[str, Any]]:
    """
    ...
    for attempt in range(retries + 1):
        try:
            ...
            try:
                ...
                try:
                    ...
                    if len(records) > count:
                        selected_records = random.sample(records, count)  # nosec B311
                        sonarr_logger.debug(
                            f"Randomly selected {len(selected_records)} missing episodes from page {random_page}"
                        )
                        return selected_records
                    else:
                        # If we have fewer episodes than requested, return all of them
                        sonarr_logger.debug(
                            f"Returning all {len(records)} missing episodes from page {random_page} (fewer than requested {count})"
                        )
                        return records

                except json.JSONDecodeError as jde:
                    sonarr_logger.error(
                        f"Failed to decode JSON response for missing episodes page {random_page}: {str(jde)}"
                    )
                    if attempt < retries:
                        time.sleep(retry_delay)
                        continue
            ...
            sonarr_logger.error(
                f"Unexpected error getting missing episodes (attempt {attempt + 1}): {str(e)}", exc_info=True
            )
            if attempt < retries:
                time.sleep(retry_delay)
                continue
...
def get_command_status(
    api_url: str, api_key: str, api_timeout: int, command_id: Union[int, str]
    ...
    for attempt in range(retries + 1):
        try:
            ...
            response = requests.get(
                endpoint, headers={"X-Api-Key": api_key}, params={"includeSeries": "false"}, timeout=api_timeout
            )
            response.raise_for_status()
...
def search_season(
    ...
    try:
        ...
        command_id = response.json().get("id")
        sonarr_logger.info(
            f"Triggered Sonarr season search for series ID: {series_id}, season: {season_number}. Command ID: {command_id}"
        )
        return command_id
    except requests.exceptions.RequestException as e:
        sonarr_logger.error(
            f"Error triggering Sonarr season search for series ID {series_id}, season {season_number}: {e}"
        )
        return None
    except Exception as e:
...
def get_cutoff_unmet_episodes_for_series(
    ...
    sonarr_logger.debug(
        f"Fetching cutoff unmet episodes for series ID {series_id} using direct API filter (monitored_only={monitored_only})"
    )
    ...
    while True:
        ...
        while retry_count <= retries_per_page and not success:
            ...
            url = f"{api_url}/api/v3/{endpoint}"
            sonarr_logger.debug(
                f"Requesting cutoff unmet page {page} for series {series_id} (attempt {retry_count + 1}/{retries_per_page + 1})"
            )
    ...
    if monitored_only:
        ...
        sonarr_logger.debug(
            f"Filtered for monitored_only=True: {len(filtered_episodes)} monitored episodes (out of {original_count} total)"
        )
...
def get_series_with_missing_episodes(
    api_url: str, api_key: str, api_timeout: int, monitored_only: bool = True, limit: int = 50, random_mode: bool = True
) -> List[Dict[str, Any]]:
    """
    ...
    for series in filtered_series[:limit]:
        ...
        try:
            ...
            missing_episodes = [
                e for e in episodes if e.get("hasFile") is False and (not monitored_only or e.get("monitored", False))
            ]
            ...
            if seasons_dict:
                missing_info = {
                    ...
                    "seasons": [
                        {"season_number": season, "episode_count": len(episodes), "episodes": episodes}
                ...
                sonarr_logger.debug(
                    f"Found series {series_title} with {len(missing_episodes)} missing episodes across {len(seasons_dict)} seasons"
                )

        except Exception as e:
            sonarr_logger.error(
                f"Error checking missing episodes for series {series_title} (ID: {series_id}): {str(e)}"
            )
            continue

    selection_mode = "RANDOM" if random_mode else "SEQUENTIAL"
    sonarr_logger.info(
        f"Examined {examined_count} series ({selection_mode} mode) and found {len(series_with_missing)} with missing episodes"
    )
