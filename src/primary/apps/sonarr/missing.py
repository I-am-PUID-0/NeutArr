#!/usr/bin/env python3
"""
Sonarr missing episodes processing module for NeutArr
"""
...
def process_missing_episodes(
    ...
    if hunt_missing_mode == "episodes":
        ...
        sonarr_logger.error(
            f"Invalid hunt_missing_mode: {hunt_missing_mode}. Valid options are 'episodes', 'seasons_packs', or 'shows'."
        )
        return False
...
def process_missing_episodes_mode(
    ...
    sonarr_logger.info(
        f"Found {len(unprocessed_episodes)} unprocessed missing episodes out of {len(episodes_to_search)} total."
    )
    ...
    for series_id, episode_ids in series_to_refresh.items():
        ...
        sonarr_logger.info(
            f"Processing series: {series_title} (ID: {series_id}) with {len(episode_ids)} missing episodes."
        )
        ...
        if search_command_id:
            ...
            if wait_for_command(
                ...
                sonarr_logger.warning(
                    f"Episode search command (ID: {search_command_id}) for series {series_id} did not complete successfully or timed out. Episodes will not be marked as processed yet."
                )
        else:
...
def process_missing_seasons_packs_mode(
    ...
    sonarr_logger.info(
        f"Found {len(unprocessed_seasons)} unprocessed seasons with missing episodes out of {len(seasons_list)} total."
    )
    ...
    if unprocessed_seasons and hunt_missing_items > 0:
        ...
        sonarr_logger.info(
            f"Randomly selected {min(len(unprocessed_seasons), hunt_missing_items)} seasons with missing episodes:"
        )

        for idx, season in enumerate(seasons_to_process):
            sonarr_logger.info(
                f"  {idx + 1}. {season['series_title']} - Season {season['season_number']} ({season['episode_count']} missing episodes) (Series ID: {season['series_id']})"
            )
    ...
    for season in unprocessed_seasons:
        ...
        sonarr_logger.info(
            f"Searching for season pack: {series_title} - Season {season_number} (contains {episode_count} missing episodes)"
        )
        ...
        if command_id:
            ...
            sonarr_logger.debug(
                f"Added season ID {season_id} to processed list for {instance_name}, success: {success}"
            )
...
def process_missing_shows_mode(
    ...
    sonarr_logger.info(
        f"Found {len(unprocessed_series)} unprocessed series with missing episodes out of {len(series_with_missing)} total."
    )
    ...
    shows_to_process = random.sample(unprocessed_series, min(len(unprocessed_series), hunt_missing_items))  # nosec B311
    ...
    for show in shows_to_process:
        ...
        if skip_future_episodes:
            ...
            missing_episodes = [
                ...
                and time.mktime(time.strptime(ep["airDateUtc"], "%Y-%m-%dT%H:%M:%SZ")) < now_unix
            ]
