...
def process_missing_items(
    ...
    if skip_future_releases:
        ...
        missing_items = [
            ...
            if not item.get("airDateUtc")
            or (
                item.get("airDateUtc")
                and datetime.datetime.fromisoformat(item["airDateUtc"].replace("Z", "+00:00")) < now
            )
        ]
    ...
    whisparr_logger.info(
        f"Found {len(unprocessed_items)} unprocessed items out of {len(missing_items)} total items with missing files."
    )

    if not unprocessed_items:
        whisparr_logger.info(
            f"No unprocessed items found for {instance_name}. All available items have been processed."
        )
        return False
    ...
    items_to_search = random.sample(unprocessed_items, min(len(unprocessed_items), hunt_missing_items))  # nosec B311
