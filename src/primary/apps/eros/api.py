...
def get_items_with_missing(
    api_url: str, api_key: str, api_timeout: int, monitored_only: bool, search_mode: str = "movie"
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        if search_mode == "movie":
            ...
            if isinstance(response, list):
                ...
                items = [
                    item for item in response if item.get("hasFile", False) and item.get("qualityCutoffNotMet", False)
                ]
            elif isinstance(response, dict) and "records" in response:
                # Fallback to old format if somehow it returns in this format
                items = [
                    ...
                    if item.get("hasFile", False) and item.get("qualityCutoffNotMet", False)
                ]
