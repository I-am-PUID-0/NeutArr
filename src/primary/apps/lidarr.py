...
def status():
    ...
    try:
        ...
        if system_status is not None:
            ...
            return jsonify({"connected": True, "version": version, "message": f"Connected to Lidarr {version}"}), 200
        else:
...
def test_connection():
    ...
    try:
        ...
        if system_status is not None:
            version = system_status.get("version", "Unknown")
            return jsonify(
                {"success": True, "version": version, "message": f"Successfully connected to Lidarr {version}"}
            ), 200
        else:
