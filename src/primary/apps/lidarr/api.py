    ...
    try:
        ...
        if response.content and response.headers.get("Content-Type", "").startswith("application/json"):
            ...
            lidarr_logger.warning(
                f"Request successful (status {response.status_code}) but no JSON content returned from {endpoint}"
            )
        ...
        lidarr_logger.error(
            f"Error decoding JSON response from Lidarr endpoint '{endpoint}'. Response: {response.text[:500]}"
        )
        return None
...
def get_artists(
    api_url: str, api_key: str, api_timeout: int, artist_id: Optional[int] = None
...
def get_albums(
    api_url: str, api_key: str, api_timeout: int, album_id: Optional[int] = None, artist_id: Optional[int] = None
) -> Union[List, Dict, None]:
...
def get_missing_albums(api_url: str, api_key: str, api_timeout: int, monitored_only: bool) -> List[Dict[str, Any]]:
    ...
    while True:
        ...
        if response and isinstance(response, dict) and "records" in response:
            ...
            lidarr_logger.debug(
                f"Parsed {total_records_on_page} missing album records from Lidarr API JSON (page {page})."
            )
            ...
            if total_records_reported >= 0 and len(all_missing_albums) >= total_records_reported:
                lidarr_logger.debug(
                    f"Fetched {len(all_missing_albums)} records, matching or exceeding total reported ({total_records_reported}). Assuming last page."
                )
                break

            if total_records_on_page < page_size:
                lidarr_logger.debug(
                    f"Received {total_records_on_page} records (less than page size {page_size}). Assuming last page."
                )
                break
    ...
    if monitored_only:
        ...
        filtered_missing = [
            album
            for album in all_missing_albums
            if album.get("monitored", False) and album.get("artist", {}).get("monitored", False)
        ]
        lidarr_logger.debug(
            f"Filtered for monitored_only=True: {len(filtered_missing)} monitored missing albums remain (out of {original_count} total)."
        )
...
def get_cutoff_unmet_albums(api_url: str, api_key: str, api_timeout: int, monitored_only: bool) -> List[Dict[str, Any]]:
    ...
    while True:
        ...
        if response and isinstance(response, dict) and "records" in response:
            ...
            lidarr_logger.debug(
                f"Parsed {total_records_on_page} cutoff unmet album records from Lidarr API JSON (page {page})."
            )
            ...
            if total_records_reported >= 0 and len(all_cutoff_unmet) >= total_records_reported:
                lidarr_logger.debug(
                    f"Fetched {len(all_cutoff_unmet)} records, matching or exceeding total reported ({total_records_reported}). Assuming last page."
                )
                break
            ...
            lidarr_logger.error(
                f"Error getting cutoff unmet albums from Lidarr (page {page}) or invalid response format. Stopping pagination."
            )
    ...
    if monitored_only:
        ...
        filtered_cutoff_unmet = [
            album
            for album in all_cutoff_unmet
            if album.get("monitored", False) and album.get("artist", {}).get("monitored", False)
        ]
        lidarr_logger.debug(
            f"Filtered for monitored_only=True: {len(filtered_cutoff_unmet)} monitored cutoff unmet albums remain (out of {original_count} total)."
        )
...
def search_artist(api_url: str, api_key: str, api_timeout: int, artist_id: int) -> Optional[Dict]:
    ...
    payload = {"name": "ArtistSearch", "artistIds": [artist_id]}
    response = arr_request(api_url, api_key, api_timeout, "command", method="POST", data=payload)
