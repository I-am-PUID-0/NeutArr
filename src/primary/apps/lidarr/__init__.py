        ...
        for idx, instance in enumerate(settings["instances"]):
            ...
            if api_url and not (api_url.startswith("http://") or api_url.startswith("https://")):
                lidarr_logger.warning(
                    f"Instance '{instance.get('name', 'Unnamed')}' has URL without http(s) scheme: {api_url}"
                )
            ...
            if is_enabled and api_url and api_key:
                ...
                if instance_name == "Default":
                    ...
                    lidarr_logger.debug(
                        f"Skipping instance '{instance_name}' due to missing API URL or key (URL: '{api_url}', Key Set: {bool(api_key)})"
                    )
                else:
