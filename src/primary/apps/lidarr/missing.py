#!/usr/bin/env python3
"""
Lidarr missing content processing module for NeutArr
...
def process_missing_albums(
    ...
    if hunt_missing_items <= 0:
        lidarr_logger.debug(
            f"Hunting for missing items is disabled (hunt_missing_items={hunt_missing_items}) for {instance_name}"
        )
        return False
    ...
    try:
        ...
        lidarr_logger.info(
            f"Found {len(missing_items)} potentially missing albums for {instance_name} after initial fetch."
        )
        ...
        if skip_future_releases:
            ...
            for item in missing_items:
                ...
                if release_date_str:
                    try:
                        ...
                        lidarr_logger.warning(
                            f"Could not parse release date '{release_date_str}' for album ID {item.get('id')}. Error: {e}. Including it."
                        )
            ...
            if skipped_count > 0:
                lidarr_logger.info(
                    f"Skipped {skipped_count} future albums based on release date. {len(missing_items)} remaining."
                )
        else:
        ...
        if hunt_missing_mode == "artist":
            ...
            unprocessed_entities = [
                eid for eid in target_entities if not is_processed("lidarr", instance_name, str(eid))
            ]

            lidarr_logger.info(
                f"Found {len(unprocessed_entities)} unprocessed artists out of {len(target_entities)} total"
            )
        else:
            ...
            lidarr_logger.info(
                f"Found {len(unprocessed_entities)} unprocessed albums out of {len(target_entities)} total"
            )

        if not unprocessed_entities:
            lidarr_logger.info(
                f"No unprocessed {search_entity_type}s found for {instance_name}. All available {search_entity_type}s have been processed."
            )
            return False
        ...
        entities_to_search_ids = random.sample(  # nosec B311
            unprocessed_entities, min(len(unprocessed_entities), total_items_to_process)
        )
        ...
        if hunt_missing_mode == "artist":
            ...
            lidarr_logger.info(
                f"Triggering Artist Search for {len(entities_to_search_ids)} artists on {instance_name}..."
            )
            for i, artist_id in enumerate(entities_to_search_ids):
                ...
                lidarr_logger.debug(
                    f"Added artist ID {artist_id} to processed list for {instance_name}, success: {success}"
                )
                ...
                lidarr_logger.info(
                    f"Triggered Lidarr ArtistSearch for artist ID: {artist_id}, Command ID: {command_id}"
                )
                ...
                if artist_id in items_by_artist:
                    for album in items_by_artist[artist_id]:
                        ...
                        if album_id:
                            ...
                            lidarr_logger.debug(
                                f"Added album ID {album_id} to processed list for {instance_name}, success: {album_success}"
                            )
            ...
            for album_id in album_ids_to_search:
                ...
                lidarr_logger.debug(
                    f"Added album ID {album_id} to processed list for {instance_name}, success: {success}"
                )
            ...
            if command_id:
                ...
                lidarr_logger.debug(
                    f"Album search command triggered with ID: {command_id} for albums: [{', '.join(album_details_log)}]"
                )
                ...
                lidarr_logger.warning(
                    f"Failed to trigger album search for IDs {album_ids_to_search} on {instance_name}."
                )

    except Exception as e:
        lidarr_logger.error(
            f"An error occurred during missing album processing for {instance_name}: {e}", exc_info=True
        )
        return False

    lidarr_logger.info(
        f"Missing album processing finished for {instance_name}. Processed {processed_count} items/searches ({len(processed_artists_or_albums)} unique {search_entity_type}s)."
    )
