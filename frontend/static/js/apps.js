...
const appsModule = {
    ...
    markAppsAsChanged: function() {
        ...
        if (currentApp) {
            ...
            if (appType) {
                ...
                // Also update the neutarrUI tracking if available
                if (window.neutarrUI && window.neutarrUI.formChanged) {
                    window.neutarrUI.formChanged[appType] = true;
                    window.neutarrUI.hasUnsavedChanges = true;
                }
            }
        }
    ...
    saveApps: function(event) {
        ...
        if (!appType) {
            ...
            if (typeof neutarrUI !== 'undefined' && typeof neutarrUI.showNotification === 'function') {
                neutarrUI.showNotification('Error: Could not determine which app settings to save', 'error');
            } else {
        ...
        if (!appPanel) {
            ...
            if (typeof neutarrUI !== 'undefined' && typeof neutarrUI.showNotification === 'function') {
                neutarrUI.showNotification(`Error: App panel not found for ${appType}`, 'error');
            } else {
    ...
    saveAppSettings: function(appType, appPanel) {
        ...
        try {
            ...
            if (typeof neutarrUI !== 'undefined' && typeof neutarrUI.showNotification === 'function') {
                neutarrUI.showNotification(`Error collecting settings: ${error.message}`, 'error');
            } else {
        ...
        .then(data => {
            ...
            if (typeof neutarrUI !== 'undefined' && typeof neutarrUI.showNotification === 'function') {
                neutarrUI.showNotification(`${appType} settings saved successfully`, 'success');
            } else {
        ...
        .catch(error => {
            ...
            if (typeof neutarrUI !== 'undefined' && typeof neutarrUI.showNotification === 'function') {
                neutarrUI.showNotification(`Error saving settings: ${error.message}`, 'error');
            } else {
    ...
    markFormAsUnchanged: function(appPanel) {
        ...
        try {
            ...
            if (appType === 'readarr' || appType === 'lidarr' || appType === 'whisparr' || appType === 'whisparrv2') {
                ...
                if (window.neutarrUI && window.neutarrUI.formChanged) {
                    window.neutarrUI.formChanged[appType] = false;
                }
                // Reset the global changed state tracker if this was the only app with changes
                if (!this.settingsChanged && window.neutarrUI) {
                    window.neutarrUI.hasUnsavedChanges = false;
                }
