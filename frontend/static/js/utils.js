...
const NeutArrUtils = {
    ...
    fetchWithTimeout: function(url, options = {}) {
        ...
        // Try to get timeout from neutarrUI if available
        if (window.neutarrUI && window.neutarrUI.originalSettings && 
            window.neutarrUI.originalSettings.general && 
            window.neutarrUI.originalSettings.general.api_timeout) {
            apiTimeout = window.neutarrUI.originalSettings.general.api_timeout * 1000;
        }
     ...
     * Prevents XSS from server-returned or user-entered strings.
    ...
    getApiTimeout: function() {
        ...
        if (window.neutarrUI && window.neutarrUI.originalSettings && 
            window.neutarrUI.originalSettings.general && 
            window.neutarrUI.originalSettings.general.api_timeout) {
            timeout = window.neutarrUI.originalSettings.general.api_timeout;
        }
