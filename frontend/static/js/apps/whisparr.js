/**
 * Whisparr.js - Handles Whisparr settings and interactions in the NeutArr UI
 */
...
function setupWhisparrForm() {
    ...
    testWhisparrButton.addEventListener('click', function() {
        ...
        if (!apiUrl || !apiKey) {
            ...
            if (typeof neutarrUI !== 'undefined' && neutarrUI.showNotification) {
                neutarrUI.showNotification('Please enter both API URL and API Key for Whisparr', 'error');
            } else {
        ...
        NeutArrUtils.fetchWithTimeout('/api/whisparr/test-connection', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (whisparrStatusIndicator) {
                if (data.success) {
                    ...
                    if (typeof neutarrUI !== 'undefined' && neutarrUI.showNotification) {
                         neutarrUI.showNotification('Successfully connected to Whisparr V2', 'success');
                    }
                    ...
                    whisparrStatusIndicator.textContent = 'Failed';
                     if (typeof neutarrUI !== 'undefined' && neutarrUI.showNotification) {
                        neutarrUI.showNotification('Connection to Whisparr failed: ' + data.message, 'error');
                    }
                }
            }
        })
        .catch(error => {
            ...
            if (typeof neutarrUI !== 'undefined' && neutarrUI.showNotification) {
                neutarrUI.showNotification('Error testing Whisparr connection: ' + error, 'error');
            }
        })
        .finally(() => {
    ...
    if (apiUrlInput && apiKeyInput && whisparrVersionDisplay && 
        ...
        const wasSettingsChanged = typeof neutarrUI !== 'undefined' ? neutarrUI.settingsChanged : false;
        
        getWhisparrVersion();
        
        // Restore the original settingsChanged state after the version check
        if (typeof neutarrUI !== 'undefined' && neutarrUI.settingsChanged !== wasSettingsChanged) {
            setTimeout(() => {
                neutarrUI.settingsChanged = wasSettingsChanged;
                ...
                if (!wasSettingsChanged && typeof neutarrUI.updateSaveResetButtonState === 'function') {
                    neutarrUI.updateSaveResetButtonState(false);
                }
            }, 100);
        }
    }
    ...
    function getWhisparrVersion() {
        ...
        NeutArrUtils.fetchWithTimeout('/api/whisparr/get-versions')
            .then(response => {
                if (!response.ok) {
