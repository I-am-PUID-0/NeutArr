/**
 * Eros.js - Handles Eros settings and interactions in the NeutArr UI
 */
    ...
    testErosButton.addEventListener('click', function() {
        ...
        if (!apiUrlInput.value || !apiKeyInput.value) {
            if (typeof neutarrUI !== 'undefined') {
                neutarrUI.showNotification('Please enter both API URL and API Key for Eros', 'error');
            } else {
        ...
        NeutArrUtils.fetchWithTimeout('/api/eros/test-connection', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            ...
            if (erosStatusIndicator) {
                if (data.success) {
                    ...
                    if (typeof neutarrUI !== 'undefined') {
                         neutarrUI.showNotification('Successfully connected to Eros', 'success');
                    }
                    ...
                    if (typeof neutarrUI !== 'undefined') {
                        neutarrUI.showNotification(data.message || 'Failed to connect to Eros', 'error');
                    } else {
        ...
        .catch(error => {
            ...
            if (typeof neutarrUI !== 'undefined') {
                neutarrUI.showNotification('Error testing connection: ' + error.message, 'error');
            } else {
