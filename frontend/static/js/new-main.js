...
let neutarrUI = {
    ...
    init: function() {
        ...
        const prefersDarkMode = localStorage.getItem('neutarr-dark-mode') === 'true';
        if (prefersDarkMode) {
        ...
        this.logoUrl = localStorage.getItem('neutarr-logo-url') || this.logoUrl;
    ...
    setupEventListeners: function() {
        ...
        if (darkModeToggle) {
            ...
            darkModeToggle.addEventListener('change', function() {
                ...
                localStorage.setItem('neutarr-dark-mode', isDarkMode);
            });
        }
    ...
    setupLogoHandling: function() {
        ...
        window.addEventListener('beforeunload', () => {
            ...
            if (this.logoSrc) {
                sessionStorage.setItem('neutarr-logo-src', this.logoSrc);
            }
        });
    },
    ...
    switchSection: function(section) {
        ...
        if (section === 'home' && this.elements.homeSection) {
            ...
            if (sponsorsFrame && (!sponsorsFrame.src || sponsorsFrame.src === 'about:blank')) { // Set src only if not already set or blank
                sponsorsFrame.src = 'https://github.com/sponsors/I-am-PUID-0';
            }
            // Disconnect logs if switching away from logs
            this.disconnectAllEventSources();
    ...
    handleSettingsOptionChange: function(e) {
        ...
        console.log(`[neutarrUI] Switched settings tab to: ${this.currentSettingsTab}`); // Added logging
    },
        ...
        try {
            ...
            eventSource.onmessage = (event) => {
                ...
                try {
                    ...
                    // Example: [SONARR] 2024-01-01 12:00:00 - neutarr.sonarr - INFO - Message content
                    // Example: 2024-01-01 12:00:00 - neutarr - DEBUG - System message
                    ...
                    if (match && match[1]) {
                        ...
                        // Otherwise try to determine from the logger name (e.g., neutarr.sonarr)
                    ...
                    if (match) {
                        ...
                        const esc = NeutArrUtils.escapeHtml;
                        const safeTimestamp = esc(timestamp);
                        const safeLevel = esc(level);
                        const safeLoggerName = esc(loggerName.replace('neutarr.', ''));
                        const safeMessage = esc(message);

                        logEntry.innerHTML = `
                            <span class="log-timestamp" title="${safeTimestamp}">${safeTimestamp.split(' ')[1]}</span>
                            ${appName ? `<span class="log-app" title="Source: ${esc(appName)}">[${esc(appName)}]</span>` : ''}
                            <span class="log-level log-level-${safeLevel.toLowerCase()}" title="Level: ${safeLevel}">${safeLevel}</span>
                            <span class="log-logger" title="Logger: ${safeLoggerName}">(${safeLoggerName})</span>
                            <span class="log-message">${safeMessage}</span>
                        `;
                        logEntry.classList.add(`log-${level.toLowerCase()}`);
                    } else {
                        // Fallback for lines that don't match the expected format
                        logEntry.innerHTML = `<span class="log-message">${NeutArrUtils.escapeHtml(logString)}</span>`;
                    ...
                    console.error('[neutarrUI] Error processing log message:', error, 'Data:', event.data);
                }
            };
            
            eventSource.onerror = (err) => {
                console.error(`[neutarrUI] EventSource error for app ${this.currentLogApp}:`, err);
                ...
                if (this.eventSources.logs) {
                    this.eventSources.logs.close();
                    console.log(`[neutarrUI] Closed potentially broken log EventSource for ${this.currentLogApp}.`);
                }
                // Attempt to reconnect after a delay, but only if still on the logs page
                if (this.currentSection === 'logs') {
                    console.log(`[neutarrUI] Attempting to reconnect log stream for ${this.currentLogApp} in 5 seconds...`);
                    setTimeout(() => {
                        // Double-check if still on logs page before reconnecting
                        if (this.currentSection === 'logs') {
                             console.log(`[neutarrUI] Reconnecting log stream for ${this.currentLogApp}.`);
                             this.connectToLogs(); // Re-initiate connection
                        } else {
                             console.log(`[neutarrUI] Log reconnect cancelled; user navigated away from logs section.`);
                        }
                    }, 5000); // 5-second delay
                }
            ...
            console.error(`[neutarrUI] Failed to create EventSource for app ${appType}:`, e);
            if (this.elements.logConnectionStatus) {
            ...
            if (source) {
                 try {
                     if (source.readyState !== EventSource.CLOSED) {
                         source.close();
                         console.log(`[neutarrUI] Closed event source for ${key}.`);
                     } else {
                         console.log(`[neutarrUI] Event source for ${key} was already closed.`);
                     }
                 } catch (e) {
                     console.error(`[neutarrUI] Error closing event source for ${key}:`, e);
                 }
            }
            // Clear the reference
    ...
    loadAllSettings: function() {
        ...
        NeutArrUtils.fetchWithTimeout('/api/settings')
            ...
            if (typeof formFunction === 'function') {
                ...
                if (typeof SettingsForms.updateDurationDisplay === 'function') {
                    try {
                        ...
                        console.error(`[neutarrUI] Error updating duration display:`, e);
                    }
                }
            } else {
                console.error(`[neutarrUI] Form generator function not found for app: ${app}`);
            }
        } else {
            console.error('[neutarrUI] SettingsForms is not defined');
            return;
        }
    },
    ...
    markSettingsAsChanged() {
        if (!this.settingsChanged) {
            console.log("[neutarrUI] Settings marked as changed.");
    ...
    saveSettings: function() {
        ...
        if (!settings) {
            console.error(`[neutarrUI] Failed to collect settings for app: ${app}`);
            this.showNotification('Error collecting settings from form.', 'error');
            return;
        }

        console.log(`[neutarrUI] Collected settings for ${app}:`, settings);
        ...
        console.log(`[neutarrUI] Local access bypass changed: ${isLocalAccessBypassChanged}`);
        console.log(`[neutarrUI] Proxy auth bypass changed: ${isProxyAuthBypassChanged}`);

        console.log(`[neutarrUI] Sending settings payload for ${app}:`, settings);

        // Use the correct endpoint based on app type
        const endpoint = app === 'general' ? '/api/settings/general' : `/api/settings/${app}`;
        
        NeutArrUtils.fetchWithTimeout(endpoint, {
            method: 'POST',
            headers: {
        ...
        .then(savedConfig => {
            console.log('[neutarrUI] Settings saved successfully:', savedConfig);
    ...
    getFormSettings: function(app) {
        ...
        const form = document.getElementById(`${app}Settings`);
        if (!form) {
            console.error(`[neutarrUI] Settings form for ${app} not found.`);
            return null;
        }
        ...
        if (instanceItems.length > 0) {
            console.log(`[neutarrUI] Found ${instanceItems.length} instance items for ${app}. Processing multi-instance mode.`);
            ...
            console.log(`[neutarrUI] No instance items found for ${app}. Processing single-instance mode.`);
        ...
        console.log(`[neutarrUI] Final collected settings for ${app}:`, settings);
        return settings;
    },
    ...
    testInstanceConnection: function(appName, instanceId, url, apiKey) {
        ...
        NeutArrUtils.fetchWithTimeout(`/api/${appName}/test-connection`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    updateConnectionStatus: function(app, statusData) {
        ...
        if (!appBox) {
            ...
            console.warn(`[neutarrUI] Could not find parent '.app-stats-card' element for ${app}`);
        }
    ...
    // User actions
    startHunt: function() {
        NeutArrUtils.fetchWithTimeout('/api/hunt/start', { method: 'POST' })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
    ...
    stopHunt: function() {
        NeutArrUtils.fetchWithTimeout('/api/hunt/stop', { method: 'POST' })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
    ...
    logout: function(e) { // Added logout function
        ...
        .then(data => {
            if (data.success) {
                console.log('[neutarrUI] Logout successful, redirecting to login.');
                window.location.href = '/login'; // Redirect to login page
            } else {
                console.error('[neutarrUI] Logout failed:', data.message);
    ...
    loadMediaStats: function() {
        NeutArrUtils.fetchWithTimeout('/api/stats')
            .then(response => {
                if (!response.ok) {
    ...
    resetMediaStats: function(appType = null) {
        ...
        try {
            ...
            NeutArrUtils.fetchWithTimeout('/api/stats/reset_public', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    loadLatestVersion: function() {
        return;
        NeutArrUtils.fetchWithTimeout('https://api.github.com/repos/I-am-PUID-0/NeutArr/releases/latest')
            .then(response => {
                if (!response.ok) {
    ...
    loadBetaVersion: function() {
        return;
        NeutArrUtils.fetchWithTimeout('https://api.github.com/repos/I-am-PUID-0/NeutArr/tags?per_page=100')
            .then(response => {
                if (!response.ok) {
                    // Handle rate limiting or other errors
                    if (response.status === 403) {
            ...
            .then(data => {
                ...
                if (betaVersionElement && data && Array.isArray(data) && data.length > 0) {
                    ...
                    if (betaTag) {
                        ...
                        try {
                            const versionInfo = localStorage.getItem('neutarr-version-info') || '{}';
                            const parsedInfo = JSON.parse(versionInfo);
                            parsedInfo.betaVersion = betaTag.name;
                            localStorage.setItem('neutarr-version-info', JSON.stringify(parsedInfo));
                        } catch (e) {
    ...
    loadGitHubStarCount: function() {
        ...
        const apiUrl = 'https://api.github.com/repos/I-am-PUID-0/NeutArr';
        
        NeutArrUtils.fetchWithTimeout(apiUrl)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                if (data && data.stargazers_count !== undefined) {
                    ...
                    localStorage.setItem('neutarr-github-stars', JSON.stringify(cacheData));
                } else {
            ...
            .catch(error => {
                ...
                const cachedData = localStorage.getItem('neutarr-github-stars');
                if (cachedData) {
                    try {
    ...
    updateHomeConnectionStatus: function() {
        console.log('[neutarrUI] Updating home connection statuses...');
    ...
    loadStatefulInfo: function(attempts = 0, skipCache = false) {
        ...
        const cachedStatefulData = localStorage.getItem('neutarr-stateful-data');
        ...
        NeutArrUtils.fetchWithTimeout('/api/stateful/info', { 
            cache: 'no-cache',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                localStorage.setItem('neutarr-stateful-data', JSON.stringify({
                    ...data,
                    timestamp: Date.now()
                }));
    ...
    resetStatefulManagement: function() {
        ...
        NeutArrUtils.fetchWithTimeout('/api/stateful/reset', {
            method: 'POST',
            headers: {
    ...
    updateStatefulExpirationOnUI: function() {
        ...
        NeutArrUtils.fetchWithTimeout(cleanedUrl, {
            method: 'POST',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
        ...
        .then(data => {
            if (data.success) {
                console.log('[neutarrUI] Stateful expiration updated successfully:', data);
    ...
    updateStatefulExpiration: function(hours) {
        if (!hours || typeof hours !== 'number' || hours <= 0) {
            console.error('[neutarrUI] Invalid hours value for updateStatefulExpiration:', hours);
            return;
        }
        
        console.log(`[neutarrUI] Directly updating stateful expiration to ${hours} hours`);
        
        // Make a direct API call to update the stateful expiration
        NeutArrUtils.fetchWithTimeout('/api/stateful/update-expiration', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        .catch(error => {
            console.error('[neutarrUI] Error updating stateful expiration:', error);
        });
    },
    ...
    // Add resetAppCycle function to the neutarrUI object
...
// Expose neutarrUI to the global scope for access by app modules
window.neutarrUI = neutarrUI;
