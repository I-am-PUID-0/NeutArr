/**
 * Scheduling functionality for NeutArr
...
window.neutarrSchedules = window.neutarrSchedules || {
    global: [],
    sonarr: [],
    radarr: [],
    lidarr: [],
...
(function() {
    ...
    const schedules = window.neutarrSchedules;
...
function setupEventListeners() {
    ...
    if (!window.neutarrSchedulerInitialized) {
        ...
        window.neutarrSchedulerInitialized = true;
        ...
        fetch('/api/scheduler/save', {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (window.neutarrUI && typeof window.neutarrUI.showNotification === 'function') {
                        neutarrUI.showNotification('Schedules saved successfully!', 'success');
                    } else {
...
function addSchedule() {
    ...
    if (isNaN(hour) || isNaN(minute)) {
        if (window.neutarrUI && typeof window.neutarrUI.showNotification === 'function') {
            neutarrUI.showNotification('Please enter a valid hour and minute.', 'error');
        } else {
    ...
    if (!anyDaySelected) {
        // Assuming neutarrUI is globally available
        if (window.neutarrUI && typeof window.neutarrUI.showNotification === 'function') {
            neutarrUI.showNotification('Please select at least one day to save the schedule.', 'error');
        } else {
